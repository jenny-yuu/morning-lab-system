...
def promote_to_member(db: Session, line_id: str, name: str, phone: str, address: str):
...
def create_order(db: Session, user_id: str, items: str, price: int, date: str):
    db_order = models.Order(user_id=user_id, items=items, total_price=price, delivery_date=date)
    db.add(db_order)
    db.commit()
    db.refresh(db_order)
    return db_order
